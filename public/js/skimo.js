        ...
        if (rawFile.readyState === 4) {
            if (rawFile.status === 200 || rawFile.status == 0) {
                ...
                // split into an array right after the time
                    ...
                    .map(pair => {
                        return {"time": pair[0] || "", "text": pair[1] || ""}
                    })
...
*/
// Constants
let slideSearchIndex = 0;
let slideSearchResults = [];
let slidesLength = document.querySelector(".slides").children.length;

// DOM References
let banner = document.querySelector("header");
let searchModal = document.getElementById("searchModal");
let searchBar = document.getElementById("searchBar");
let moveSearchBack = document.getElementById("moveSearchBack");
let moveSearchForward = document.getElementById("moveSearchForward");
let searchSlideNumber = document.getElementById("searchSlideNumber");
...
function updateSearchResults() {
    ...
    if (isEmpty(query)) {
        ...
        let subSlides = getSlidesFromSubtitleQuery(query);

        let combinedSlides = [...subSlides, ...slideSearchResults];
